...
function calcRentStatus(
  shop: MallShop,
  payments: MallRentPayment[]
): {
  label: string
  color: string
  ...
  if (!shop.is_occupied)
    return { label: 'Vacant', color: 'bg-gray-600', remainingMonths: 0, daysUntilDue: 0 }
  if (shop.monthly_rent <= 0)
    return { label: 'No rent due', color: 'bg-gray-500', remainingMonths: 0, daysUntilDue: 0 }
  if (payments.length === 0)
    return { label: 'No payment', color: 'bg-red-600', remainingMonths: 0, daysUntilDue: 0 }
  ...
  if (remainingMonths >= 1)
    return {
      label: `${remainingMonths}mo left`,
      color: 'bg-green-600',
      remainingMonths,
      daysUntilDue,
    }
  if (daysUntilDue >= 14)
    return { label: `${daysUntilDue}d left`, color: 'bg-green-600', remainingMonths, daysUntilDue }
  if (daysUntilDue >= 7)
    return { label: `${daysUntilDue}d left`, color: 'bg-yellow-500', remainingMonths, daysUntilDue }
  if (daysUntilDue >= 1)
    return { label: `${daysUntilDue}d left`, color: 'bg-orange-500', remainingMonths, daysUntilDue }
...
export default function MallManagement({ onBack }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<
    'overview' | 'floor-plan' | 'tenants' | 'reports' | 'maintenance' | '3d-view'
  >('overview')
  ...
  const fetchData = async () => {
    ...
    const { data: shopsData } = await supabase.from('mall_shops').select('*').order('shop_number')
    if (shopsData) {
  ...
  const currentFloorsShops = shops.filter(
    (s) =>
  ...
  const handleResizeStart = useCallback(
    (e: React.MouseEvent, shopId: string, dir: string) => {
      e.preventDefault()
      e.stopPropagation()
  ...
  const handleDragStart = useCallback(
    (e: React.MouseEvent, shopId: string) => {
      if (
        (e.target as HTMLElement).closest('.resize-handle') ||
        (e.target as HTMLElement).closest('.info-btn')
      )
        return
      e.preventDefault()
      const shop = shops.find((s) => s.id === shopId)
  ...
  const saveShop = async () => {
    ...
    try {
      const payload = {
        ...
        width: Math.max(20, shopForm.width),
        height: Math.max(20, shopForm.height),
      ...
      if (editingShop) {
        ...
        await audit({
          action: 'MALL_SHOP_UPDATED',
          entity: 'mall_shop',
          entityId: editingShop.id,
          entityName: shopForm.shop_number.trim(),
  ...
  const toggleOccupied = async (shop: MallShop) => {
    ...
    await audit({
      action: next ? 'MALL_SHOP_OCCUPIED' : 'MALL_SHOP_VACATED',
      entity: 'mall_shop',
      entityId: shop.id,
      entityName: shop.shop_number,
      performer: profile as Profile,
    })
  ...
  const deleteShop = async (shop: MallShop) => {
    if (!confirm(`Delete shop ${shop.shop_number}?`)) return
    const { error } = await supabase.from('mall_shops').delete().eq('id', shop.id)
    if (error) return toast.error('Error', error.message)
    await audit({
      action: 'MALL_SHOP_DELETED',
      entity: 'mall_shop',
      entityId: shop.id,
      entityName: shop.shop_number,
      performer: profile as Profile,
  ...
  const saveRent = async () => {
    ...
    try {
      ...
      if (error) throw error
      await audit({
        action: 'MALL_RENT_PAID',
        entity: 'mall_rent_payment',
        entityName: rentShop.shop_number,
        newValue: { months, amount, shop: rentShop.shop_number },
        performer: profile as Profile,
      })
      toast.success('Recorded', `Rent recorded for ${rentShop.shop_number}`)
  ...
  const overviewStats = (() => {
    ...
    const totalMonthlyRent = shops
      .filter((s) => s.is_occupied)
      .reduce((sum, s) => sum + s.monthly_rent, 0)
    const totalDeposits = shops
      .filter((s) => s.is_occupied)
    ...
    const rentCollectedThisMonth = Object.values(rentPayments)
      .flat()
      .filter((p) => {
  ...
  const recentPayments = Object.entries(rentPayments)
    .flatMap(([shopId, payments]) =>
  ...
  // ── Tenants data ──
  const tenantList = shops
    .filter((s) => s.is_occupied || s.tenant_name)
    .map((s) => ({
      ...s,
  ...
  const leasesExpiring = shops
    ...
    .map((s) => ({
      ...
      floorName: floors.find((f) => f.id === s.floor_id)?.name || 'Unknown',
      daysLeft: Math.ceil(
        (new Date(s.lease_end_date!).getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
      ),
    }))
  ...
  const maintenanceStats = (() => {
    ...
    const urgent = maintenanceRequests.filter(
      (r) => r.priority === 'urgent' && r.status !== 'closed' && r.status !== 'resolved'
    ).length
  ...
  const exportCSV = (filename: string, headers: string[], rows: string[][]) => {
    ...
    window.alert(
      `CSV data for "${filename}" logged to console.\n\nPreview:\n${rows
        .slice(0, 3)
        .map((r) => r.join(' | '))
        .join('\n')}`
    )
  ...
  const updateMaintenanceStatus = async (req: MallMaintenanceRequest, newStatus: string) => {
    ...
    setMaintenanceRequests((prev) =>
      prev.map((r) =>
        r.id === req.id
          ? ({ ...r, ...payload, updated_at: new Date().toISOString() } as MallMaintenanceRequest)
          : r
      )
    )
  }
  ...
  const generateInvoices = async () => {
    ...
    try {
      ...
      const periodStart = new Date(nowGen.getFullYear(), nowGen.getMonth(), 1)
        .toISOString()
        .split('T')[0]
      const periodEnd = new Date(nowGen.getFullYear(), nowGen.getMonth() + 1, 0)
        .toISOString()
        .split('T')[0]
      ...
      toast.success(
        'Invoices',
        `${count} invoice(s) generated for ${nowGen.toLocaleString('default', { month: 'long', year: 'numeric' })}`
      )
  ...
  const calculateLateFees = async () => {
    try {
      const overdue = rentInvoices.filter(
        (i) => i.status === 'pending' && new Date(i.period_end) < new Date()
      )
      let count = 0
      ...
      toast.success(
        'Late Fees',
        `${count} invoice(s) updated with late fees (5% per month overdue)`
      )
      await fetchRentInvoices()
    } catch (err) {
      toast.error('Error', (err as Error).message)
  ...
  return (
    <div className="min-h-full bg-gray-950">
      <div className="p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm mt-0.5">
                Manage shops, floor plan & rent tracking
              </p>
            </div>
          </div>
        </div>
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                <div className="flex items-center gap-3 mb-3">
                  <div className="bg-gray-600/10 p-2.5 rounded-xl">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                ...
                <p className="text-white text-2xl font-bold">
                  {fmtUSDshort(overviewStats.totalMonthlyRent)}
                </p>
              </div>
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                <p className="text-gray-500 text-xs font-medium uppercase tracking-wide mb-1">
                ...
                <p className="text-green-400 text-2xl font-bold">
                  {fmtUSDshort(overviewStats.rentCollectedThisMonth)}
                </p>
              </div>
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                <p className="text-gray-500 text-xs font-medium uppercase tracking-wide mb-1">
                ...
                <p
                  className={`text-2xl font-bold ${overviewStats.overdueCount > 0 ? 'text-red-400' : 'text-white'}`}
                >
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                ...
                <p className="text-white text-2xl font-bold">
                  {fmtUSDshort(overviewStats.totalDeposits)}
                </p>
              </div>
            </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-white text-sm font-medium">
                              {p.shop?.shop_number || 'Unknown'}
                            </p>
                            <p className="text-gray-500 text-xs">
                              {p.months_paid} month{p.months_paid > 1 ? 's' : ''} ·{' '}
                              {new Date(p.paid_at).toLocaleDateString()}
                            </p>
                          </div>
                        </div>
                        <span className="text-green-400 font-medium text-sm">
                          {fmtUSD(p.amount_paid)}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-4 overflow-auto">
                <div className="flex items-center gap-3 mb-3 text-[10px] text-gray-500">
                  ...
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-sm bg-green-600 inline-block" /> Paid
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-sm bg-yellow-500 inline-block" /> Due 7-14d
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-sm bg-orange-500 inline-block" /> Due &lt;7d
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-sm bg-red-600 inline-block" /> Overdue
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-2 h-2 rounded-sm bg-gray-600 inline-block" /> Vacant
                  </span>
                </div>
                <div
                  ref={planRef}
                  ...
                  {currentFloorsShops.map((shop) => {
                    ...
                    return (
                      ...
                      >
                        {/* Occupancy stripe */}
                        <div
                          className={`absolute top-0 left-0 right-0 h-1 rounded-t-lg ${shop.is_occupied ? 'bg-green-500' : 'bg-gray-600'}`}
                        />
                        ...
                        <div
                          className="resize-handle absolute -top-1 -left-1 w-3 h-3 cursor-nwse-resize bg-white/20 hover:bg-white/40 rounded-sm z-20"
                          onMouseDown={(e) => handleResizeStart(e, shop.id, 'nw')}
                        />
                        <div
                          className="resize-handle absolute -top-1 left-1/2 -translate-x-1/2 w-3 h-3 cursor-ns-resize bg-white/20 hover:bg-white/40 rounded-sm z-20"
                          onMouseDown={(e) => handleResizeStart(e, shop.id, 'n')}
                        />
                        <div
                          className="resize-handle absolute -top-1 -right-1 w-3 h-3 cursor-nesw-resize bg-white/20 hover:bg-white/40 rounded-sm z-20"
                          onMouseDown={(e) => handleResizeStart(e, shop.id, 'ne')}
                        />
                        <div
                          className="resize-handle absolute top-1/2 -right-1 -translate-y-1/2 w-3 h-3 cursor-ew-resize bg-white/20 hover:bg-white/40 rounded-sm z-20"
                          onMouseDown={(e) => handleResizeStart(e, shop.id, 'e')}
                        />
                        <div
                          className="resize-handle absolute -bottom-1 -right-1 w-3 h-3 cursor-nwse-resize bg-white/20 hover:bg-white/40 rounded-sm z-20"
                          onMouseDown={(e) => handleResizeStart(e, shop.id, 'se')}
                        />
                        <div
                          className="resize-handle absolute -bottom-1 left-1/2 -translate-x-1/2 w-3 h-3 cursor-ns-resize bg-white/20 hover:bg-white/40 rounded-sm z-20"
                          onMouseDown={(e) => handleResizeStart(e, shop.id, 's')}
                        />
                        <div
                          className="resize-handle absolute -bottom-1 -left-1 w-3 h-3 cursor-nesw-resize bg-white/20 hover:bg-white/40 rounded-sm z-20"
                          onMouseDown={(e) => handleResizeStart(e, shop.id, 'sw')}
                        />
                        <div
                          className="resize-handle absolute top-1/2 -left-1 -translate-y-1/2 w-3 h-3 cursor-ew-resize bg-white/20 hover:bg-white/40 rounded-sm z-20"
                          onMouseDown={(e) => handleResizeStart(e, shop.id, 'w')}
                        />
                      </div>
                    )
                  })}
                ...
                {currentFloorsShops.map((shop) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-gray-300 text-xs font-medium">{shop.shop_number}</p>
                      {shop.is_occupied && shop.tenant_name && (
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-gray-400 text-xs">
                          {fmtUSDshort(shop.monthly_rent)}/mo
                        </span>
                      </div>
                ...
                {currentFloorsShops.length === 0 && (
                  <div className="col-span-full text-center py-12">
                    <p className="text-gray-500 text-sm">
                      No shops on this floor. Add your first shop.
                    </p>
                  </div>
                )}
              </div>
                ...
                return (
                  <div className="mt-4 bg-gray-900 border border-gray-800 rounded-2xl p-5">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h3 className="text-white font-bold text-lg">Shop {shop.shop_number}</h3>
                        <p className="text-gray-400 text-sm">{activeFloorObj?.name || ''}</p>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                      ...
                      <div className="bg-gray-800 rounded-xl p-3">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          ...
                          <span className="text-white font-medium text-sm">
                            {shop.is_occupied ? 'Occupied' : 'Vacant'} — {status.label}
                          </span>
                        </div>
                      </div>
                      <div className="bg-gray-800 rounded-xl p-3">
                        ...
                        <p className="text-white font-medium text-sm">
                          {shop.tenant_name || 'N/A'}
                        </p>
                        {shop.tenant_phone && (
                          <p className="text-gray-400 text-xs mt-0.5">{shop.tenant_phone}</p>
                        )}
                      </div>
                      <div className="bg-gray-800 rounded-xl p-3">
                        <p className="text-gray-500 text-xs">Monthly Rent</p>
                        <p className="text-white font-medium text-sm">
                          {fmtUSDshort(shop.monthly_rent)}
                        </p>
                      </div>
                      <div className="bg-gray-800 rounded-xl p-3">
                        <p className="text-gray-500 text-xs">Position</p>
                        <p className="text-white font-medium text-sm">
                          X: {shop.pos_x}px Y: {shop.pos_y}px
                        </p>
                        <p className="text-gray-400 text-xs">
                          {shop.width} x {shop.height}px
                        </p>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        ...
                        {shop.lease_start_date && (
                          <div className="bg-gray-800 rounded-xl p-3">
                            ...
                            <p className="text-white font-medium text-sm">
                              {new Date(shop.lease_start_date).toLocaleDateString()} —{' '}
                              {shop.lease_end_date
                                ? new Date(shop.lease_end_date).toLocaleDateString()
                                : 'Open'}
                            </p>
                          </div>
                        )}
                        {shop.deposit_amount > 0 && (
                          <div className="bg-gray-800 rounded-xl p-3">
                            ...
                            <p className="text-white font-medium text-sm">
                              {fmtUSDshort(shop.deposit_amount)}
                            </p>
                          </div>
                        )}
                      </div>
                      ...
                      {payments.length === 0 ? (
                        ...
                        <div className="space-y-2 max-h-48 overflow-y-auto">
                          {[...payments].reverse().map((payment) => (
                            <div
                              key={payment.id}
                              className="bg-gray-800 rounded-xl px-4 py-2.5 flex items-center justify-between"
                            >
                              <div>
                                <p className="text-white text-sm font-medium">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() =>
                    exportCSV(
                      'occupancy-report',
                      ['Floor', 'Total Shops', 'Occupied', 'Vacant', 'Occupancy %'],
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-800">
                      <th className="text-left text-gray-400 font-medium px-3 py-2.5 text-xs uppercase tracking-wide">
                        Floor
                      </th>
                      <th className="text-right text-gray-400 font-medium px-3 py-2.5 text-xs uppercase tracking-wide">
                        Total Shops
                      </th>
                      <th className="text-right text-gray-400 font-medium px-3 py-2.5 text-xs uppercase tracking-wide">
                        Occupied
                    ...
                    {occupancyReport.map((r) => (
                      <tr key={r.floor} className="hover:bg-gray-800/30">
                        ...
                        <td className="px-3 py-2.5 text-right">
                          <span
                            className={`font-medium ${r.rate >= 80 ? 'text-green-400' : r.rate >= 50 ? 'text-amber-400' : 'text-red-400'}`}
                          >
                            {r.rate}%
                          </span>
                        </td>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() =>
                    exportCSV(
                      'rent-collection',
                      ['Floor', 'Expected', 'Collected', 'Outstanding'],
                      ...
                      return (
                        <tr key={r.floor} className="hover:bg-gray-800/30">
                          ...
                          <td className="px-3 py-2.5 text-gray-300 text-right">
                            {fmtUSDshort(r.expected)}
                          </td>
                          <td className="px-3 py-2.5 text-green-400 text-right">
                            {fmtUSDshort(r.collected)}
                          </td>
                          <td className="px-3 py-2.5 text-red-400 text-right">
                            {r.outstanding > 0 ? fmtUSDshort(r.outstanding) : '$0'}
                          </td>
                          <td className="px-3 py-2.5 text-right">
                            <span
                              className={`font-medium ${pct >= 90 ? 'text-green-400' : pct >= 50 ? 'text-amber-400' : 'text-red-400'}`}
                            >
                              {pct}%
                            </span>
                          </td>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  onClick={() =>
                    exportCSV(
                      'leases-expiring',
                      ['Shop #', 'Floor', 'Tenant', 'Lease End', 'Days Left'],
                      leasesExpiring.map((r) => [
                        r.shop_number,
                        r.floorName,
                        r.tenant_name || '',
                        r.lease_end_date ? new Date(r.lease_end_date).toLocaleDateString() : '',
                        String(r.daysLeft),
                      ])
                    )
                  }
                      ...
                      {leasesExpiring.map((r) => (
                        <tr key={r.id} className="hover:bg-gray-800/30">
                          ...
                          <td className="px-3 py-2.5 text-gray-300">
                            {new Date(r.lease_end_date!).toLocaleDateString()}
                          </td>
                          <td className="px-3 py-2.5 text-right">
                            <span
                              className={`font-medium ${r.daysLeft <= 7 ? 'text-red-400' : r.daysLeft <= 30 ? 'text-amber-400' : 'text-yellow-400'}`}
                            >
                              {r.daysLeft}d
                            </span>
                          </td>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                ...
                <p
                  className={`text-2xl font-bold ${maintenanceStats.urgent > 0 ? 'text-red-400' : 'text-white'}`}
                >
        ...
        {activeTab === 'reports' &&
          (() => {
            const totalPending = rentInvoices
              .filter((i) => i.status === 'pending')
              .reduce((s, i) => s + i.total_amount, 0)
            const totalOverdue = rentInvoices
              .filter((i) => i.status === 'overdue')
              .reduce((s, i) => s + i.total_amount, 0)
            const totalPaid = rentInvoices
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {generatingInvoices ? (
                        ...
                        <>
                          <Plus size={12} /> Generate Invoices
                        </>
                      )}
                    </button>
                  </div>
                ...
                <div className="overflow-x-auto max-h-64 overflow-y-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-gray-800">
                        <th className="text-left text-gray-400 font-medium px-3 py-2 text-xs uppercase tracking-wide">
                          Invoice #
                        </th>
                        <th className="text-left text-gray-400 font-medium px-3 py-2 text-xs uppercase tracking-wide">
                          Shop
                        ...
                        <th className="text-right text-gray-400 font-medium px-3 py-2 text-xs uppercase tracking-wide">
                          Rent
                        </th>
                        <th className="text-right text-gray-400 font-medium px-3 py-2 text-xs uppercase tracking-wide">
                          Late Fee
                        ...
                        return (
                          <tr key={inv.id} className="hover:bg-gray-800/30">
                            ...
                            <td className="px-3 py-2 text-right text-gray-300 text-xs">
                              {fmtUSDshort(inv.rent_amount)}
                            </td>
                            <td className="px-3 py-2 text-right text-xs">
                              {inv.late_fee > 0 ? (
                                <span className="text-red-400">{fmtUSDshort(inv.late_fee)}</span>
                              ) : (
                                '-'
                              )}
                            </td>
                            <td className="px-3 py-2 text-right text-white font-medium text-xs">
                              {fmtUSDshort(inv.total_amount)}
                            </td>
                      ...
                      {rentInvoices.length === 0 && (
                        <tr>
                          <td colSpan={7} className="text-center py-8 text-gray-500 text-xs">
                            No invoices generated yet. Click "Generate Invoices" to create invoices
              ...
              <div className="p-5 space-y-4">
                  ...
                  <div>
                    <label className="flex items-center gap-2 cursor-pointer mt-6">
                      <input
                        ...
                        onChange={(e) =>
                          setShopForm({ ...shopForm, is_occupied: e.target.checked })
                        }
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="number"
                      value={shopForm.pos_x}
                      onChange={(e) =>
                        setShopForm({ ...shopForm, pos_x: parseInt(e.target.value) || 0 })
                      }
                      className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={shopForm.pos_y}
                      onChange={(e) =>
                        setShopForm({ ...shopForm, pos_y: parseInt(e.target.value) || 0 })
                      }
                      className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="number"
                      min={20}
                      value={shopForm.width}
                      onChange={(e) =>
                        setShopForm({ ...shopForm, width: parseInt(e.target.value) || 20 })
                      }
                      className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      min={20}
                      value={shopForm.height}
                      onChange={(e) =>
                        setShopForm({ ...shopForm, height: parseInt(e.target.value) || 20 })
                      }
                      className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                    />
                  </div>
                ...
                <div className="border-t border-gray-800 pt-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        value={shopForm.tenant_phone}
                        onChange={(e) => setShopForm({ ...shopForm, tenant_phone: e.target.value })}
                ...
                <div className="border-t border-gray-800 pt-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setShopForm({ ...shopForm, monthly_rent: e.target.value })}
                    ...
                    <div>
                      ...
                      <input
                        value={shopForm.deposit_amount}
                        onChange={(e) =>
                          setShopForm({ ...shopForm, deposit_amount: e.target.value })
                        }
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-3">
                    <div>
                      ...
                      <input
                        type="date"
                        value={shopForm.lease_start_date}
                        onChange={(e) =>
                          setShopForm({ ...shopForm, lease_start_date: e.target.value })
                        }
                        className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="date"
                        value={shopForm.lease_end_date}
                        onChange={(e) =>
                          setShopForm({ ...shopForm, lease_end_date: e.target.value })
                        }
                        className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                      />
                    </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Additional notes about this shop or tenant"
                    rows={3}
                    className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                  />
                </div>
        ...
        {showRentForm && rentShop && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-900 rounded-2xl w-full max-w-sm border border-gray-800">
              <div className="flex items-center justify-between p-5 border-b border-gray-800">
                <h3 className="text-white font-bold">Rent Payment — {rentShop.shop_number}</h3>
                <button
                  onClick={() => {
                    setShowRentForm(false)
                    setRentShop(null)
              ...
              <div className="p-5 space-y-4">
                <div>
                  ...
                  <input
                    type="number"
                    min={1}
                    value={rentForm.months_paid}
                    onChange={(e) => setRentForm({ ...rentForm, months_paid: e.target.value })}
                    className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    value={rentForm.amount_paid}
                    onChange={(e) => setRentForm({ ...rentForm, amount_paid: e.target.value })}
                    placeholder="e.g. 1500"
                    className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    disabled
                    className="w-full bg-gray-800/50 border border-gray-700 text-gray-400 rounded-xl px-3 py-2 text-sm cursor-not-allowed"
                  />
                </div>
                <div>
                  <label className="text-gray-400 text-xs font-medium block mb-1">Notes</label>
              ...
              <div className="p-5 border-t border-gray-800 flex gap-2 justify-end">
                ...
                <button
                  onClick={saveRent}
                  disabled={savingRent}
                  className="bg-green-600 hover:bg-green-500 disabled:bg-gray-700 disabled:text-gray-500 text-white font-bold px-4 py-2 rounded-xl text-sm transition-colors"
                >
              ...
              <div className="p-5 space-y-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setMaintenanceForm({ ...maintenanceForm, shop_id: e.target.value })
                    }
                    className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-amber-500"
                  >
                  ...
                  <div className="p-5 space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-gray-800 rounded-xl p-4">
                        ...
                        <p className="text-white font-semibold">{shop.shop_number}</p>
                        <p className="text-gray-400 text-xs mt-0.5">
                          {floors.find((f) => f.id === shop.floor_id)?.name || 'Unknown'}
                        </p>
                      </div>
                      <div className="bg-gray-800 rounded-xl p-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        ...
                        <div className="bg-gray-800 rounded-xl p-3">
                          <p className="text-gray-500 text-xs">Deposit</p>
                          <p className="text-white text-sm font-medium mt-0.5">
                            {shop.deposit_amount > 0 ? fmtUSDshort(shop.deposit_amount) : '-'}
                          </p>
                        </div>
                        <div className="bg-gray-800 rounded-xl p-3">
                          ...
                          {[...payments].reverse().map((p) => (
                            ...
                            >
                              <div>
                                ...
                                {p.notes && (
                                  <p className="text-gray-500 text-xs mt-0.5">{p.notes}</p>
                                )}
                                {p.recorded_by_name && (
                                  <p className="text-gray-600 text-xs mt-0.5">
